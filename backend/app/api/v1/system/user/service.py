...
class UserService:
    ...
    @staticmethod
    async def get_button_permissions() -> typing.List[str]:
        """根据 token 获取当前用户按钮权限码。"""
        ...
        if not user_info:
            raise ValueError(CodeEnum.PARTNER_CODE_TOKEN_EXPIRED_FAIL.msg)
        return await PermissionService.get_permission_codes(user_info)
