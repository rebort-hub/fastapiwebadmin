...
from app.api.v1.system.lookup.schema import LookupIn, LookupValueQuery, LookupValueIn, LookupQuery, LookupId
from app.api.v1.system.lookup.service import LookupValueService, LookupService
from app.utils.response import HttpResponse

LookupRouter = APIRouter(prefix="/lookup", tags=["数据字典"])


@LookupRouter.post('/getAllLookup', description="获取所有数据字典")
async def get_all_lookup():
    data = await LookupValueService.get_all_lookup()
    return await HttpResponse.success(data)


@LookupRouter.post('/getLookupList', description="获取数据字典列表")
async def lookup_list(params: LookupQuery):
    data = await LookupService.list(params)
    return await HttpResponse.success(data)


@LookupRouter.post('/saveOrUpdateLookup', description="新增或更新字典")
async def save_or_update_lookup(params: LookupIn):
    data = await LookupService.save_or_update(params)
    return await HttpResponse.success(data)


@LookupRouter.post('/delLookup', description="删除字典")
async def del_lookup(params: LookupId):
    data = await LookupService.deleted(params)
    return await HttpResponse.success(data)


@LookupRouter.post('/getLookupValue', description="获取字典值")
...
@LookupRouter.post('/saveOrUpdateLookupValue', description="保存或更新字典值")
...
@LookupRouter.post('/delLookupValue', description="删除字典值")
